...
# plot(x=hpc.days$Time, y=hpc.days$Global_active_power, type = "l", xlab = "", ylab = "Global Active Power (kilowatts)")
# plot(x=hpc.days$Time, y=hpc.days$Sub_metering_1, type = "l", col="black", bg = "transparent", xlab = "", ylab = "Energy sub metering")
# lines(x=hpc.days$Time, y=hpc.days$Sub_metering_2, type = "l", col="red")
# lines(x=hpc.days$Time, y=hpc.days$Sub_metering_3, type = "l", col="blue")
# legend("topright", legend = c("Sub_metering_1", "Sub_metering_2","Sub_metering_3"), col = c("black","red","blue"), lty = 1)

par(mfcol=c(2,2))
with(hpc.days, {
        plot(x=Time, y=Global_active_power, type = "l", xlab = "", ylab = "Global Active Power", bg = "transparent")
        plot(x=Time, y=Sub_metering_1, type = "l", col="black", bg = "transparent", xlab = "", ylab = "Energy sub metering")
        lines(x=Time, y=Sub_metering_2, type = "l", col="red")
        lines(x=Time, y=Sub_metering_3, type = "l", col="blue")
        legend("topright", legend = c("Sub_metering_1", "Sub_metering_2","Sub_metering_3"), col = c("black","red","blue"), lty = 1)
        plot(x=Time, y=Voltage, type = "l", col="black", bg = "transparent", xlab = "datetime", ylab = "Voltage")
        plot(x=Time, y=Global_reactive_power, type = "l", col="black", bg = "transparent", xlab = "datetime", ylab = "Global_reactive_power")
